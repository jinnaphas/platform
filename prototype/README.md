...
โหมดออฟไลน์: ข้อมูลของแต่ละทีมเก็บแยกกันใน `localStorage` ของเครื่องผู้ใช้

## โหมด Online (💾 Save ร่วมกันทั้ง 6 ทีม)

เชื่อมกับ Cloudflare Worker (`worker/worker.js`) ที่ commit ข้อมูลลง `data/<team>.json`
ใน repo นี้ — ผู้ใช้ยืนยันตัวด้วย "รหัสทีม" ไม่ต้องมีบัญชี GitHub, ทุกการ Save เป็น
commit จริง (มี version history), มีระบบกันบันทึกทับกันด้วย SHA + สำรองงานอัตโนมัติ
เมื่อชนกัน และ poll ของใหม่ทุก 20 วินาที → ดูขั้นตอนติดตั้งทั้งหมดใน [`SETUP.md`](../SETUP.md)
